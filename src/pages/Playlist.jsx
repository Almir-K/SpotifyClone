		...
		<Box id="Playlist__page" sx={{ backgroundColor: ' background.paper', flex: 1, overflowY: 'auto' }}>
			<Box
				...
				sx={{
					...
					alignItems: { xs: 'flex-end', md: 'flex-end', xl: 'center' },
					gap: 3,
			...
			>
				{status.isLoading ? (
					<Skeleton
						sx={{ variant: 'square', width: { xs: ' 100%', md: 235 }, height: { xs: ' 100%', md: 235 } }}
					/>
				) : (
					<Avatar
				...
				<Box>
					...
					{status.isLoading ? (
						<Skeleton variant="texts" sx={{ fontSize: { xs: 42, md: 72 }, width: 200 }} />
					) : (
						<Typography sx={{ fontSize: { xs: 42, md: 72 }, fontWeight: 'bold', color: 'primary' }}>
							{playlistInfo?.name}
						</Typography>
					)}
				</Box>
